...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    _loadLockStatusAndAuthenticate();
  ...
  Future<void> _loadLockStatusAndAuthenticate() async {
    ...
    if (_lockEnabled) {
      bool success = await _authenticateIfLocked();
      if (!success) {
        // Authentication failed and navigation already handled inside _authenticateIfLocked
        // Just return here to stop init further or show locked UI if you want.
        return;
      }
    }
  }
  ...
  Future<void> _loadLockPref() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() => _lockEnabled = prefs.getBool('lock_enabled') ?? false);
  }
